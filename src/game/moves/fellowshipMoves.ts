...
export const revealCompanion = (
    ...
    fpPlayer.fellowshipArea = fpPlayer.fellowshipArea.map((card) => {
        const isTarget =
            card.id === companionId || (card as any).instanceId === companionId;
        if (isTarget) {
            found = true;
...
export const playSite = (
    ...
    if (siteIndex === -1) {
        console.warn(
            `❌ [moves.playSite] Site ${siteId} introuvable dans le deck de sites.`
        );
        return 'INVALID_MOVE';
    }
...
export const endFellowshipPhase = ({
    G,
    ctx,
    events,
    playerID,
